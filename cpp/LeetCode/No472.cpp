...
public:
    vector<string> findAllConcatenatedWordsInADict(vector<string>& words) {
        ...
        for(string& word : words){
            if(isConcatenated(word, 0, root, root)) res.push_back(word);
            root->insert(word);
        }
        return res;
    }
private:
    bool isConcatenated(string& word, int index, Trie* root, Trie* node){
        if(index==word.size()) return node->isWord;

        if(node->children.count(word[index])==0) return false;
        else{
            if(isConcatenated(word, index+1, root, node->children[word[index]])) return true;
            if(node->children[word[index]]->isWord && isConcatenated(word, index+1, root, root)) return true;
        }
        return false;
    }
};
