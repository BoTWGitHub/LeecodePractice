...
public:
    int maxProfit(int k, vector<int>& prices) {
        vector<vector<vector<int>>> mem(prices.size(), vector<vector<int>>(k+1, vector<int>(2, -1)));
        return dp(k, prices, 0, false, mem);
    }
private:
    int dp(int k, vector<int> prices, int index, bool holding, vector<vector<vector<int>>> &mem){
        if(k<=0) return 0;
        if(index==prices.size()) return 0;
        if(mem[index][k][holding]!=-1) return mem[index][k][holding];

        int dosomething = 0;
        int donothing = dp(k, prices, index+1, holding, mem);
        if(!holding){
            dosomething = dp(k, prices, index+1, true, mem) - prices[index];
        }
        else{
            dosomething = dp(k-1, prices, index+1, false, mem) + prices[index];
        }
        mem[index][k][holding] = max(dosomething, donothing);
        return mem[index][k][holding];
    }
};
