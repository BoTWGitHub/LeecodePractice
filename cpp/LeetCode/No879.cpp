...
public:
    int profitableSchemes(int n, int minProfit, vector<int>& group, vector<int>& profit) {
        vector<vector<vector<int>>> mem(n+1, vector<vector<int>>(group.size(), vector<int>(minProfit+1, -1)));
        return dp(n, minProfit, group, profit, 0, mem);
    }
private:
    const int mod = 1e9+7;
    int dp(int n, int minProfit, vector<int>& group, vector<int>& profit, int index, vector<vector<vector<int>>>& mem){
        if(n==0) return minProfit<=0;
        if(index==group.size()) return minProfit<=0;
        if(mem[n][index][minProfit]!=-1) return mem[n][index][minProfit];

        int res = dp(n, minProfit, group, profit, index+1, mem)%mod;
        if(n>=group[index]) res = (res + dp(n-group[index], max(0, minProfit-profit[index]), group, profit, index+1, mem))%mod;

        return mem[n][index][minProfit] = res;
    }
};
